...
class JointViDataset(Dataset):
    ...
    def get_mapped_duration2metas(self):
        ...
        if total_metas <= 0:
            raise ValueError(
                'No valid official T2IV metas after filtering. '
                f'total_rows={total}, corrupt={corrupt}, train_max_token_len={self.train_max_token_len}, '
                ...
                'Increase TRAIN_MAX_TOKEN_LEN/DENSE_RATIO4SEQPACK or check metadata duration/resolution.'
            )
        mapped_duration2freqs = {}
        for mapped_duration in sorted(mapped_duration2metas.keys()):
    ...
    def exists_cache_file(self, meta):
        ...
        if 'image_path' in meta:
            ...
            cache_file = self.get_video_cache_file(
                meta['video_path'], meta['begin_frame_id'], meta['end_frame_id'], self.video_fps, pn
            )
    ...
    def form_batches(self, mapped_duration2metas):
        ...
        if self.num_replicas > 1:
            if not tdist.is_initialized():
                raise RuntimeError(
                    'Official T2IV dataset requires an initialized process group when num_replicas > 1; '
                    f'got num_replicas={self.num_replicas}.'
                )
            batch_num_tensor = torch.tensor([batch_num], device=self.device, dtype=torch.int64)
            tdist.all_reduce(batch_num_tensor, op=tdist.ReduceOp.MIN)
            batch_num = int(batch_num_tensor.item())
        if batch_num <= 0:
            raise RuntimeError(
                'Official T2IV dataset produced no common batch across replicas; '
                f'local_batches={len(batches)}, num_replicas={self.num_replicas}.'
            )
        ...
        try:
            ...
            for (mapped_duration, example_ind) in batch_info:
                ...
                while not ret:
                    ...
                    else: # Try the next example when decoding fails.
                        example_ind = example_ind + 1
                        repeat_times += 1
                        if repeat_times % 20 == 0: # Switch batches after repeated decoding failures.
