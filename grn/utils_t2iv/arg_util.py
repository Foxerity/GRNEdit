...
class Args(Tap):
    ...
    seed: int = 0                       # synchronized runtime seed; persisted for strict mid-epoch resume
    block_chunks: int = 0               # exact registered transformer chunk count; persisted for strict edit resumes
    ...
    min_tlr: float = 0.0                # lower bound for official edit step-wise decay
    lr_decay_steps: int = 1000          # optimizer-update interval for official edit decay
    lr_decay_gamma: float = 0.5         # multiplicative official edit decay
    ...
    t5_max_tokens: int = 512            # T5-only truncation length when extra text conditions are appended
    use_reprompt_text: int = 0          # use precomputed reprompt text from official edit metadata
    ...
    save_token_cache: int = 0           # whether write shared token/text caches
    ...
    video_caption_type: str = ''        # metadata field containing the edit instruction
    ...
    alpha: float = 1.0
    # GRNEdit runtime fields are serialized in training checkpoints.
    t5_enable_fsdp: int = 1
    stage15_identity_prob: float = 0.15
    stage15_empty_prompt: str = ''
    stage15_use_reprompt_text: int = 0
    stage15_source_residual_modulation: str = 'text_pt'
    stage15_source_injection_mask: str = ''
    stage15_text_schema: str = ''
    stage15_branch_lr: float = 0.0
    stage15_branch_min_lr: float = 0.0
    stage15_branch_warmup_start_lr: float = 0.0
    stage15_branch_warmup_steps: int = 0
    stage15_branch_lr_decay_steps: int = 0
    stage15_branch_lr_decay_gamma: float = 1.0
    stage15_main_lr: float = 0.0
    stage15_main_min_lr: float = 0.0
    stage15_main_warmup_start_lr: float = 0.0
    stage15_main_warmup_steps: int = 0
    stage15_main_lr_decay_steps: int = 0
    stage15_main_lr_decay_gamma: float = 1.0
    stage15_main_freeze_steps: int = 0
    stage15_resume_policy: str = ''
    stage15_world_size: int = 0
    edit_data_fingerprint: str = ''
    edit_metadata_file_count: int = 0
    vae_sha256: str = ''
    t5_encoder_sha256: str = ''
    t5_tokenizer_sha256: str = ''
    stage2_algorithm: str = ''
    stage2_bridge_mlp_ratio: float = 1.0
    # Backward-compatible alias used by an earlier Open checkpoint draft.
    stage2_router_mlp_ratio: float = 1.0
    stage2_base_loss_weight: float = 0.25
    stage2_delta_loss_weight: float = 1e-4
    stage2_resume_policy: str = ''
    stage2_world_size: int = 0
    stage2_freeze_main: int = 1
    refine_mode: str = ''
...
def init_dist_and_get_args(init_sequence_parallel: bool = True):
    ...
    if init_sequence_parallel and args.sp_size > 1:
