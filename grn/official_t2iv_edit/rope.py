...
def precompute_rope3d_freqs_grid(
    ...
    requested_text_len = int(text_maxlen or 0)
    if args is not None:
        requested_text_len = max(requested_text_len, int(getattr(args, 'tlen', 0) or 0))
        requested_text_len = max(
            requested_text_len,
            int(getattr(args, 'tlen', 512) or 512) + int(getattr(args, 'add_scale_token', 0) or 0),
        )
    preserve_1d_length = max(600, requested_text_len)
