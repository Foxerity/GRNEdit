...
const distDir = path.resolve(siteDir, "dist", "client");
const publicDir = path.resolve(siteDir, "public");

await rm(path.resolve(siteDir, "dist"), { recursive: true, force: true });
...
await stat(path.join(distDir, "_next"));
...
// Every source asset must be present byte-for-byte in the generated Pages
// artifact. This guards the large video collection as well as logos, posters,
// and images.
const publicManifest = await createManifest(publicDir);
const artifactManifest = await createManifest(distDir);
const assetProblems = [];

for (const [relativePath, size] of publicManifest) {
  const exportedSize = artifactManifest.get(relativePath);
...
const missingReferences = [...new Set(emittedReferences)].filter(
  (relativePath) => !artifactManifest.has(relativePath),
);
...
const summary = await summarize(distDir);
console.log(
  `GitHub Pages artifact ready in dist/client (${summary.files} files, ${(summary.bytes / 1024 / 1024).toFixed(2)} MiB; ${publicManifest.size} public assets verified).`,
);
